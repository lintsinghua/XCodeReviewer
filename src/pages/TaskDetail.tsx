...
function IssuesList({
		...
		return (
			<div className="flex items-center justify-center space-x-2 mt-6 pb-4">
				...
				<span className="text-sm text-gray-600 ml-4">
					第 {current} / {total} 页，共 {totalIssues} 条，显示{" "}
					{itemsPerPage * (current - 1) + 1}-
					{Math.min(itemsPerPage * current, totalIssues)} 条
				</span>
			</div>
		);
	};
	...
	const renderIssue = (issue: AuditIssue, index: number) => {
		...
		if (index < 3) {
			console.log(`Issue ${index}:`, {
				...
				fix_example: issue.fix_example
					? issue.fix_example.substring(0, 100) + "..."
					: "NULL",
		...
		return (
			<div
				key={issue.id || index}
				className={`bg-white border rounded-lg p-4 hover:shadow-md transition-all duration-200 group ${isSelected ? "border-primary bg-blue-50" : "border-gray-200 hover:border-gray-300"} ${isResolved ? "opacity-60" : ""}`}
			>
				<div className="flex items-start justify-between mb-3">
					<div className="flex items-start space-x-3">
						...
						<div className="flex-1">
							<h4 className="font-semibold text-base text-gray-900 mb-1 group-hover:text-gray-700 transition-colors">
								{issue.title}
							</h4>
							...
							{issue.line_number && (
								<div className="flex items-center space-x-1 text-xs text-gray-500 mt-1">
									...
									{issue.column_number && (
										<span>，第 {issue.column_number} 列</span>
									)}
								</div>
							)}
						</div>
					...
					<Badge
						className={`${getSeverityColor(issue.severity)} px-2 py-1 text-xs font-medium`}
					>
						{issue.severity === "critical"
							? "严重"
							: issue.severity === "high"
				...
				<div className="space-y-2">
								...
								<div className="bg-white border border-blue-200 rounded-lg p-3 shadow-sm">
									...
									{isCode ? (
										<pre className="bg-blue-50 border border-blue-200 rounded p-2 overflow-x-auto">
											<code className="text-xs text-blue-900 font-mono whitespace-pre">
												{issue.suggestion}
											</code>
										</pre>
									) : (
					...
					{(() => {
						const showFixExample =
							issue.fix_example && issue.fix_example.trim().length > 0;
						if (index < 3) {
						...
						return showFixExample ? (
							<div className="bg-white border border-green-200 rounded-lg p-3 shadow-sm">
								...
								<pre className="bg-gray-50 border border-gray-200 rounded p-2 overflow-x-auto">
									<code className="text-xs text-gray-800 font-mono whitespace-pre">
										{issue.fix_example}
									</code>
								</pre>
							</div>
...
export default function TaskDetail() {
	...
	const loadIssues = async () => {
		...
		try {
			const response = await api.getAuditIssues(
				id,
				currentPage,
				issuesPerPage,
				currentSeverity,
			...
			console.log("API Response:", {
				...
				items_count: Array.isArray(response)
					? response.length
					...
					<CardContent className="p-5">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="stat-value text-xl text-orange-600">
									{task.issues_count}
								</p>
							</div>
									...
									<p className="text-base flex items-center">
										...
										{task.llm_provider ? (
											<span className="flex items-center space-x-1">
												{task.llm_provider.icon && (
													<span>{task.llm_provider.icon}</span>
												)}
															...
															<div className="grid grid-cols-2 gap-2 text-xs">
																...
																{config.scan_categories && (
																	<div className="flex items-center space-x-2 col-span-2">
																		...
																		<span className="text-gray-700 font-medium">
																			{config.scan_categories.length} 个
																		</span>
																	</div>
																)}
