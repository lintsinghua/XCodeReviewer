    ...
    <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-blue-900">
              ...
              <div className="space-y-2">
                <p>
                  <strong>系统提示词（System Prompt）：</strong>定义 AI 助手的角色、行为规范和输出格式。在对话开始时发送给 AI。
                </p>
                <p>
                  <strong>用户提示词（User Prompt）：</strong>包含具体的任务内容和代码，支持占位符动态替换。在每次分析时与代码一起发送。
                </p>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              编辑系统提示词模版: {selectedTemplate && getTemplateLabel(selectedTemplate.key)}
            </DialogTitle>
          </DialogHeader>
              ...
              {selectedTemplate?.key.includes("worker_prompt") ? (
                <div className="text-xs text-blue-600 mt-2 bg-blue-50 p-2 rounded">
                  ...
                  <ul className="mt-1 ml-4 list-disc">
                    <li><code>{"{code_to_review}"}</code> - 要审查的代码内容</li>
                    <li><code>{"{context_section}"}</code> - 上下文信息（可选，自动生成）</li>
                    <li><code>{"{category}"}</code> - 扫描类别（如 SECURITY, PERFORMANCE 等）</li>
                  </ul>
                </div>
              ) : (
                <div className="text-xs text-green-600 mt-2 bg-green-50 p-2 rounded">
                  ...
                  <ul className="mt-1 ml-4 list-disc">
                    <li><code>{"{category}"}</code> - 扫描类别（如 SECURITY, PERFORMANCE 等）</li>
                    <li><code>{"{subcategories}"}</code> - 子类别描述（来自 Prompt 的 description 字段）</li>
                  </ul>
