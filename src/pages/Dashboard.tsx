	...
	const loadDashboardData = async () => {
		try {
			...
			if (results[1].status === "fulfilled") {
				setRecentProjects(
					Array.isArray(results[1].value) ? results[1].value.slice(0, 5) : [],
				);
			} else {
			...
			if (tasks.length > 0) {
				...
				const trendData = tasksByDate.map((task, index) => ({
					date: new Date(task.completed_at!).toLocaleDateString("zh-CN", {
						month: "short",
						day: "numeric",
					}),
			...
			try {
				const allIssuesResponses = await Promise.all(
					tasks.map((task) =>
						api.getAuditIssues(task.id).catch(() => ({ items: [], total: 0 })),
					),
				);
