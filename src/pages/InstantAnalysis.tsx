...
const USE_BACKEND_FOR_INSTANT_ANALYSIS =
		...
		const loadLLMProviders = async () => {
			try {
				...
				const response = await apiClient.get(
					"/llm-providers?page_size=100&is_active=true",
				);
...
class UserManager {
	...
	const handleAnalyze = async () => {
		...
		try {
			...
			if (USE_BACKEND_FOR_INSTANT_ANALYSIS) {
				...
				analysisResult = await api.analyzeInstantCode(
					code,
					language,
					selectedProviderId,
				);
	...
	const getTempTaskAndIssues = () => {
		...
		const tempIssues: AuditIssue[] = result.issues.map((issue, index) => ({
			...
			ai_explanation:
				...
				(issue.xai ? JSON.stringify(issue.xai) : undefined),
			status: "open",
	...
	const renderIssue = (issue: any, index: number) => (
		<div
			key={index}
			className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md hover:border-gray-300 transition-all duration-200 group"
		>
