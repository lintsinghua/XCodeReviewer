	...
	const loadApiKeyStatus = async () => {
		...
		try {
			...
			const response = await apiClient.get(
				`/llm-providers/${providerId}/api-key`,
			);
				...
				<div className="space-y-5 py-2">
					{loadingStatus ? (
						...
						<>
							...
							{keyStatus && (
								<div
									className={`rounded-lg border p-4 ${keyStatus.has_api_key ? "border-green-500 bg-green-50" : "border-amber-500 bg-amber-50"}`}
								>
									<div className="flex items-start gap-3">
										...
										<div className="flex-1">
											<div
												className={`font-semibold text-sm mb-1.5 ${keyStatus.has_api_key ? "text-green-900" : "text-amber-900"}`}
											>
												{keyStatus.has_api_key
													? "✓ 已配置 API Key"
													: "⚠ 未配置 API Key"}
								...
								<AlertDescription className="text-xs">
									...
									<div className="space-y-1.5 text-blue-800">
										<div className="flex items-start space-x-2">
											...
											<span>
												配置后将<strong>覆盖</strong>环境变量中的 API Key
											</span>
										</div>
										<div className="flex items-start space-x-2">
											<span className="text-blue-600 mt-0.5">•</span>
										...
										<div className="flex items-start space-x-2">
											<span className="text-blue-600 mt-0.5">•</span>
											<span>
												删除配置后将<strong>回退</strong>到使用环境变量
											</span>
										</div>
										<div className="flex items-start space-x-2">
											<span className="text-blue-600 mt-0.5">•</span>
											<span>
												仅<strong>管理员</strong>可以配置 API Key
											</span>
										</div>
									</div>
								</AlertDescription>
