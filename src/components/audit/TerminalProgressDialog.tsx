...
export default function TerminalProgressDialog({
	...
	useEffect(() => {
		...
		if (!hasInitializedLogsRef.current) {
			...
			addLog(
				`� 任务类D型: ${taskType === "repository" ? "仓库审计" : "ZIP文件审计"}`,
				"info",
			);
		...
		const pollTask = async () => {
			...
			try {
				...
				if (task.status === "pending") {
					...
					if (
						statusChanged &&
						logs.filter((l) => l.message.includes("开始扫描")).length === 0
					) {
					...
					if (issuesChanged && task.issues_count > lastIssuesCount) {
						...
						addLog(
							`⚠️  发现 ${newIssues} 个新问题 (总计: ${task.issues_count})`,
							"warning",
						);
					...
					if (linesChanged && (task.total_lines || 0) > lastTotalLines) {
						...
						addLog(
							`📝 已分析 ${(task.total_lines || 0).toLocaleString()} 行代码 [+${newLines.toLocaleString()}]`,
							"info",
						);
					...
					if (!isCompleted) {
						...
						addLog(
							`📝 总计分析: ${(task.total_lines || 0).toLocaleString()} 行代码`,
							"success",
						);
						addLog(
							`⚠️  发现问题: ${task.issues_count || 0} 个`,
							(task.issues_count || 0) > 0 ? "warning" : "success",
						);

						// 解析问题类型分布
						if (task.issues_count > 0) {
							try {
								const { api: apiImport } = await import(
						...
						addLog(
							`⭐ 质量评分: ${(task.quality_score || 0).toFixed(1)}/100`,
							"success",
						);
						addLog("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "info");
					...
					if (!isCancelled) {
						...
						addLog(`📊 完成统计:`, "info");
						addLog(
							`  • 已分析文件: ${task.scanned_files || 0}/${task.total_files || 0}`,
							"info",
						);
						addLog(`  • 发现问题: ${task.issues_count || 0} 个`, "info");
						addLog(
							`  • 代码行数: ${(task.total_lines || 0).toLocaleString()} 行`,
							"info",
						);
						addLog("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "warning");
					...
					<div className="p-6 bg-gradient-to-b from-gray-900/95 to-gray-950/95 overflow-y-auto h-[calc(100%-120px)] font-mono text-sm backdrop-blur-sm">
						<div className="space-y-2">
							{logs.map((log, index) => (
								<div
									key={index}
									className="flex items-start space-x-3 hover:bg-red-950/10 px-2 py-1 rounded transition-colors"
								>
							...
							{!isCompleted && !isFailed && (
								<div className="flex items-center space-x-2 mt-4">
									<span className="text-rose-800/70 text-xs w-20">
										[{currentTime}]
									</span>
