...
export abstract class BaseLLMAdapter implements ILLMAdapter {
	...
	async validateConfig(): Promise<boolean> {
		if (!this.config.apiKey) {
			throw new LLMError("API Key未配置", this.config.provider);
		}
		return true;
	}
		...
		return Promise.race([
			...
			new Promise<T>((_, reject) =>
				setTimeout(
					() =>
						reject(
							new LLMError(`请求超时 (${timeoutMs}ms)`, this.config.provider),
						),
					timeoutMs,
				),
			),
	...
	protected handleError(error: any, context?: string): never {
		...
		if (error.name === "AbortError" || message.includes("超时")) {
			message =
				`请求超时 (${this.config.timeout}ms)。建议：\n` +
				`1. 检查网络连接是否正常\n` +
