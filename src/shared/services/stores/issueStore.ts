			...
			request.onsuccess = () => {
				...
				if (filters?.task_id) {
					results = results.filter(
						(i: AuditIssue) => i.task_id === filters.task_id,
					);
				}
				if (filters?.severity) {
					results = results.filter(
						(i: AuditIssue) => i.severity === filters.severity,
					);
				}
				if (filters?.category) {
					results = results.filter(
						(i: AuditIssue) => i.category === filters.category,
					);
				}
				if (filters?.status) {
					results = results.filter(
						(i: AuditIssue) => i.status === filters.status,
					);
				}
	...
	updateIssue: async (id, data) => {
		...
		try {
			...
			set((state) => ({
				...
				currentIssue:
					state.currentIssue?.id === id ? issue : state.currentIssue,
				loading: false,
			}));
