...
import { useBackendTasks, useWebSocketUpdates } from '../../config/featureFlags';
import type { 
  AuditTask, 
  TaskCreate, 
  TaskUpdate, 
...
class IndexedDBTaskService {
  ...
  async getAll(filters?: { project_id?: number; status?: string }): Promise<AuditTask[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (filters?.project_id) {
          results = results.filter((t: AuditTask) => t.project_id === filters.project_id);
        }
        if (filters?.status) {
          results = results.filter((t: AuditTask) => t.status === filters.status);
        }
...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  cancelTask: async (id) => {
    ...
    try {
      if (useBackendTasks()) {
        ...
        // Use IndexedDB
        await indexedDBService.update(id, { status: 'cancelled' as TaskStatus });
  ...
  handleTaskProgress: (data) => {
    ...
    set(state => ({
      tasks: state.tasks.map(t => 
        t.id === data.task_id 
          ? { ...t, progress: data.progress, current_step: data.current_step, status: data.status }
          : t
      ),
      currentTask: state.currentTask?.id === data.task_id
        ? { ...state.currentTask, progress: data.progress, current_step: data.current_step, status: data.status }
        : state.currentTask,
    }));
  },
