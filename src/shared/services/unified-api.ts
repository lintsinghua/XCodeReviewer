...
class BackendAPIAdapter {
	...
	private transformProject(backendProject: any): Project {
		return {
			...
			repository_url:
				backendProject.repository_url || backendProject.source_url || "",
			// 后端字段名可能是 source_type 或 repository_type
			repository_type:
				backendProject.repository_type ||
				backendProject.source_type ||
				"github",
			// 后端字段名可能是 branch 或 default_branch
			default_branch:
				backendProject.default_branch || backendProject.branch || "main",
			// 处理 programming_languages 字段（后端可能返回数组或字符串）
			programming_languages:
				typeof backendProject.programming_languages === "string"
					? backendProject.programming_languages
					: JSON.stringify(backendProject.programming_languages || []),
			owner_id:
				backendProject.owner_id || backendProject.created_by || "unknown",
	...
	private transformTask(backendTask: any): AuditTask {
		return {
			...
			project: backendTask.project
				? this.transformProject(backendTask.project)
				: undefined,
			...
			quality_score:
				backendTask.overall_score || backendTask.quality_score || 0,
			exclude_patterns:
				typeof backendTask.exclude_patterns === "string"
					? backendTask.exclude_patterns
					: JSON.stringify(backendTask.exclude_patterns || []),
			scan_config:
				typeof backendTask.scan_config === "string"
	...
	private transformIssue(backendIssue: any): AuditIssue {
		return {
			...
			task: backendIssue.task
				? this.transformTask(backendIssue.task)
				: undefined,
			...
			column_number:
				backendIssue.column_start || backendIssue.column_number || null,
	...
	async getProjects(): Promise<Project[]> {
		...
		return Array.isArray(projects)
			? projects.map((p) => this.transformProject(p))
			: [];
	}
	...
	async updateProject(
		...
		const backendUpdates: any = {
			...
			...(updates.repository_url !== undefined && {
				source_url: updates.repository_url,
			}),
		...
		if (updates.repository_url) {
			backendUpdates.repository_name = this.extractRepoName(
				updates.repository_url,
			);
		}

		const updated = await backendApi.projects.update(
			Number(id),
			backendUpdates,
		);
	...
	async getDeletedProjects(): Promise<Project[]> {
		try {
			const response = await backendApi.projects.listDeleted({
				page: 1,
				page_size: 100,
			});
	...
	async getAuditIssues(
		...
		return {
			items: Array.isArray(issues)
				? issues.map((i) => this.transformIssue(i))
				: [],
	...
	async createAuditIssue(
		issue: Omit<AuditIssue, "id" | "created_at" | "task" | "resolver">,
	): Promise<AuditIssue> {
	 ...
	 * 即时代码分析（通过后端LLM）
	 */
	async analyzeInstantCode(
		code: string,
		language: string,
		llm_provider_id?: number,
	): Promise<CodeAnalysisResult> {
		const result = await backendApi.instantAnalysis.analyze({
			code,
			language,
			llm_provider_id,
		});
...
export const unifiedApi = {
	// 项目相关
	getProjects: () =>
		USE_BACKEND ? backendAdapter.getProjects() : localApi.getProjects(),
	getProjectById: (id: string) =>
		USE_BACKEND
			? backendAdapter.getProjectById(id)
			: localApi.getProjectById(id),
	createProject: (project: CreateProjectForm & { owner_id?: string }) =>
		USE_BACKEND
			? backendAdapter.createProject(project)
			: localApi.createProject(project),
	updateProject: (id: string, updates: Partial<CreateProjectForm>) =>
		USE_BACKEND
			? backendAdapter.updateProject(id, updates)
			: localApi.updateProject(id, updates),
	deleteProject: (id: string) =>
		USE_BACKEND ? backendAdapter.deleteProject(id) : localApi.deleteProject(id),
	getDeletedProjects: () =>
		USE_BACKEND
			? backendAdapter.getDeletedProjects()
			: localApi.getDeletedProjects(),
	restoreProject: (id: string) =>
		USE_BACKEND
			? backendAdapter.restoreProject(id)
			: localApi.restoreProject(id),
	permanentlyDeleteProject: (id: string) =>
		USE_BACKEND
			? backendAdapter.permanentlyDeleteProject(id)
			: localApi.permanentlyDeleteProject(id),

	// 任务相关
	getAuditTasks: (projectId?: string) =>
		USE_BACKEND
			? backendAdapter.getAuditTasks(projectId)
			: localApi.getAuditTasks(projectId),
	getAuditTaskById: (id: string) =>
		USE_BACKEND
			? backendAdapter.getAuditTaskById(id)
			: localApi.getAuditTaskById(id),
	createAuditTask: (task: CreateAuditTaskForm & { created_by: string }) =>
		USE_BACKEND
			? backendAdapter.createAuditTask(task)
			: localApi.createAuditTask(task),
	updateAuditTask: (id: string, updates: Partial<AuditTask>) =>
		USE_BACKEND
			? backendAdapter.updateAuditTask(id, updates)
			: localApi.updateAuditTask(id, updates),
	...
	getAuditIssues: (
		...
		USE_BACKEND
			? backendAdapter.getAuditIssues(taskId, page, pageSize, severity, status)
			: localApi.getAuditIssues(taskId),
	createAuditIssue: (
		issue: Omit<AuditIssue, "id" | "created_at" | "task" | "resolver">,
	) =>
		USE_BACKEND
			? backendAdapter.createAuditIssue(issue)
			: localApi.createAuditIssue(issue),
	updateAuditIssue: (id: string, updates: Partial<AuditIssue>) =>
		USE_BACKEND
			? backendAdapter.updateAuditIssue(id, updates)
			: localApi.updateAuditIssue(id, updates),
	bulkUpdateIssues: (issueIds: number[], status: string) =>
		USE_BACKEND
			? backendAdapter.bulkUpdateIssues(issueIds, status)
			: Promise.reject("Not supported in local mode"),

	// 统计相关
	getProjectStats: () =>
		USE_BACKEND ? backendAdapter.getProjectStats() : localApi.getProjectStats(),

	// 用户相关
	getProfilesById: (id: string) =>
		USE_BACKEND
			? backendAdapter.getProfilesById(id)
			: localApi.getProfilesById(id),
	getProfilesCount: () =>
		USE_BACKEND
			? backendAdapter.getProfilesCount()
			: localApi.getProfilesCount(),
	createProfiles: (profile: Partial<Profile>) =>
		USE_BACKEND
			? backendAdapter.createProfiles(profile)
			: localApi.createProfiles(profile),
	updateProfile: (id: string, updates: Partial<Profile>) =>
		USE_BACKEND
			? backendAdapter.updateProfile(id, updates)
			: localApi.updateProfile(id, updates),
	getAllProfiles: () =>
		USE_BACKEND ? backendAdapter.getAllProfiles() : localApi.getAllProfiles(),

	// 项目成员相关
	getProjectMembers: (projectId: string) =>
		USE_BACKEND
			? backendAdapter.getProjectMembers(projectId)
			: localApi.getProjectMembers(projectId),
	addProjectMember: (
		projectId: string,
		userId: string,
		role: string = "member",
		...
		USE_BACKEND
			? backendAdapter.addProjectMember(projectId, userId, role)
			: localApi.addProjectMember(projectId, userId, role),

	// 即时分析相关
	getInstantAnalyses: (userId?: string) =>
		USE_BACKEND
			? backendAdapter.getInstantAnalyses(userId)
			: localApi.getInstantAnalyses(userId),
	createInstantAnalysis: (
		...
		USE_BACKEND
			? backendAdapter.createInstantAnalysis(analysis)
			: localApi.createInstantAnalysis(analysis),
	...
	analyzeInstantCode: (
		...
		language: string,
		llm_provider_id?: number,
	) => backendAdapter.analyzeInstantCode(code, language, llm_provider_id),
