  ...
  private async syncTask(operation: SyncOperation): Promise<void> {
    switch (operation.type) {
      ...
      case 'delete':
        ...
        if (!operation.entityId) throw new Error('Entity ID required for cancel');
  ...
  private async syncIssue(operation: SyncOperation): Promise<void> {
    switch (operation.type) {
      case 'update':
        ...
        break;
      default:
        throw new Error(`Unsupported operation type for issues: ${operation.type}`);
    }
  }
    ...
    switch (strategy) {
      ...
      case 'merge':
        ...
        resolution.resolvedData = {
          ...
          updated_at: new Date(localData.updated_at) > new Date(remoteData.updated_at)
