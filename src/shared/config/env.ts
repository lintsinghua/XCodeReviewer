...
export const env = {
	...
	LLM_PROVIDER:
		runtimeConfig?.llmProvider || import.meta.env.VITE_LLM_PROVIDER || "gemini",
	// LLM API Key
	LLM_API_KEY:
		runtimeConfig?.llmApiKey || import.meta.env.VITE_LLM_API_KEY || "",
	// LLM 模型名称
	LLM_MODEL: runtimeConfig?.llmModel || import.meta.env.VITE_LLM_MODEL || "",
	// LLM API 基础URL (可选，用于自定义端点或代理)
	LLM_BASE_URL:
		runtimeConfig?.llmBaseUrl || import.meta.env.VITE_LLM_BASE_URL || "",
	// LLM 请求超时时间(ms)
	LLM_TIMEOUT:
		runtimeConfig?.llmTimeout ||
		Number(import.meta.env.VITE_LLM_TIMEOUT) ||
		150000,
	// LLM 温度参数 (0.0-2.0)
	LLM_TEMPERATURE:
		runtimeConfig?.llmTemperature !== undefined
			? runtimeConfig.llmTemperature
			: Number(import.meta.env.VITE_LLM_TEMPERATURE) || 0.2,
	// LLM 最大token数
	LLM_MAX_TOKENS:
	...
	LLM_CUSTOM_HEADERS:
		runtimeConfig?.llmCustomHeaders ||
		import.meta.env.VITE_LLM_CUSTOM_HEADERS ||
		"",

	// ==================== Gemini AI 配置 (兼容旧配置) ====================
	GEMINI_API_KEY:
		runtimeConfig?.geminiApiKey || import.meta.env.VITE_GEMINI_API_KEY || "",
	...
	// ==================== 智谱AI 配置 ====================
	ZHIPU_API_KEY:
		runtimeConfig?.zhipuApiKey || import.meta.env.VITE_ZHIPU_API_KEY || "",
	ZHIPU_MODEL: import.meta.env.VITE_ZHIPU_MODEL || "glm-4-flash",

	// ==================== Moonshot 配置 ====================
	MOONSHOT_API_KEY:
		runtimeConfig?.moonshotApiKey ||
	...
	// ==================== 百度文心一言 配置 ====================
	BAIDU_API_KEY:
		runtimeConfig?.baiduApiKey || import.meta.env.VITE_BAIDU_API_KEY || "",
	BAIDU_MODEL: import.meta.env.VITE_BAIDU_MODEL || "ERNIE-3.5-8K",

	// ==================== MiniMax 配置 ====================
	MINIMAX_API_KEY:
		runtimeConfig?.minimaxApiKey || import.meta.env.VITE_MINIMAX_API_KEY || "",
	MINIMAX_MODEL: import.meta.env.VITE_MINIMAX_MODEL || "abab6.5-chat",

	// ==================== 豆包 配置 ====================
	DOUBAO_API_KEY:
		runtimeConfig?.doubaoApiKey || import.meta.env.VITE_DOUBAO_API_KEY || "",
	...
	// ==================== GitHub 配置 ====================
	GITHUB_TOKEN:
		runtimeConfig?.githubToken || import.meta.env.VITE_GITHUB_TOKEN || "",

	// ==================== GitLab 配置 ====================
	GITLAB_TOKEN:
		runtimeConfig?.gitlabToken || import.meta.env.VITE_GITLAB_TOKEN || "",
	...
	// ==================== 分析配置 ====================
	MAX_ANALYZE_FILES:
		runtimeConfig?.maxAnalyzeFiles ||
		Number(import.meta.env.VITE_MAX_ANALYZE_FILES) ||
		40,
	LLM_CONCURRENCY:
		runtimeConfig?.llmConcurrency ||
		Number(import.meta.env.VITE_LLM_CONCURRENCY) ||
		2,
	LLM_GAP_MS:
		runtimeConfig?.llmGapMs || Number(import.meta.env.VITE_LLM_GAP_MS) || 500,

	// ==================== 语言配置 ====================
	OUTPUT_LANGUAGE:
		runtimeConfig?.outputLanguage ||
		import.meta.env.VITE_OUTPUT_LANGUAGE ||
		"zh-CN", // zh-CN | en-US
