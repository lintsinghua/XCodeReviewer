...
const finalSupabaseUrl = hasSupabaseConfig
	? supabaseUrl
...
export const dbMode = useLocalDB
	? "local"
	: hasSupabaseConfig
		? "supabase"
		: "demo";
...
export const api = {
	...
	async getAuditTasks(projectId?: string): Promise<AuditTask[]> {
		...
		let query = supabase.from("audit_tasks").select(`
        *,
	...
	async getInstantAnalyses(userId?: string): Promise<InstantAnalysis[]> {
		...
		let query = supabase.from("instant_analyses").select(`
        *,
