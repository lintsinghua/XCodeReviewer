					...
					try {
						...
						for (const file of limitedFiles) {
							...
							if (taskControl.isCancelled(taskId)) {
								console.log(
									`🛑 [检查点1] 任务 ${taskId} 已被用户取消（${scannedFiles}/${totalFiles} 完成），停止分析`,
								);
								await api.updateAuditTask(taskId, {
									status: "cancelled",
									total_files: totalFiles,
							...
							try {
								...
								if (taskControl.isCancelled(taskId)) {
									console.log(
										`🛑 [检查点2] 任务 ${taskId} 在LLM分析完成后检测到取消，跳过保存结果（文件: ${file.path}）`,
									);
									await api.updateAuditTask(taskId, {
										status: "cancelled",
										total_files: totalFiles,
								...
								// 每分析一个文件都更新进度，确保实时性
								console.log(
									`📈 ZIP任务 ${taskId}: 进度 ${scannedFiles}/${totalFiles} (${Math.round((scannedFiles / totalFiles) * 100)}%)`,
								);
								await api.updateAuditTask(taskId, {
									status: "running",
									total_files: totalFiles,
								...
								console.error(
									`❌ 分析文件 ${file.path} 失败 (${failedFiles}/${scannedFiles}):`,
									analysisError,
								);
								...
								if (
									...
									console.warn(
										`⏳ API频率限制！等待${waitTime / 1000}秒后继续... (已失败: ${failedFiles}次)`,
									);
								...
								// 更新进度（即使失败也要显示进度）
								console.log(
									`📈 ZIP任务 ${taskId}: 进度 ${scannedFiles}/${totalFiles} (${Math.round((scannedFiles / totalFiles) * 100)}%) - 失败: ${failedFiles}`,
								);
								await api.updateAuditTask(taskId, {
									status: "running",
									total_files: totalFiles,
						...
						console.log(
							`📊 扫描完成统计: 总计${totalFiles}个文件, 成功${scannedFiles - failedFiles}个, 失败${failedFiles}个, 成功率${successRate.toFixed(1)}%`,
						);

						if (failedFiles > 0 && failedFiles < totalFiles) {
							console.warn(
								`⚠️ 部分文件分析失败，但任务标记为完成。建议检查.env配置或更换LLM提供商`,
							);
						}
