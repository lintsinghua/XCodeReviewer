...
const isTextFile = (p: string) =>
	TEXT_EXTENSIONS.some((ext) => p.toLowerCase().endsWith(ext));
const matchExclude = (p: string, ex: string[]) =>
	ex.some(
		(e) =>
			p.includes(e.replace(/^\//, "")) ||
			(e.endsWith("/**") && p.startsWith(e.slice(0, -3).replace(/^\//, ""))),
	);
...
async function githubApi<T>(url: string, token?: string): Promise<T> {
	...
	if (!res.ok) {
		if (res.status === 403)
			throw new Error(
				"GitHub API 403：请配置 VITE_GITHUB_TOKEN 或确认仓库权限/频率限制",
			);
...
async function gitlabApi<T>(url: string, token?: string): Promise<T> {
	...
	if (!res.ok) {
		if (res.status === 401)
			throw new Error(
				"GitLab API 401：请配置 VITE_GITLAB_TOKEN 或确认仓库权限",
			);
		if (res.status === 403)
			throw new Error("GitLab API 403：请确认仓库权限/频率限制");
	...
	(async () => {
		...
		try {
			...
			if (isGitHub) {
				...
				if (!m)
					throw new Error(
						"GitHub 仓库 URL 格式错误，例如 https://github.com/owner/repo",
					);
				const owner = m[1];
				const repo = m[2];

				const treeUrl = `https://api.github.com/repos/${owner}/${repo}/git/trees/${encodeURIComponent(branch)}?recursive=1`;
				const tree = await githubApi<{ tree: GithubTreeItem[] }>(
					treeUrl,
					params.githubToken,
				);
				files = (tree.tree || [])
					.filter(
						(i) =>
							i.type === "blob" &&
							isTextFile(i.path) &&
							!matchExclude(i.path, excludes),
					)
					.map((i) => ({
						path: i.path,
						url: `https://raw.githubusercontent.com/${owner}/${repo}/${encodeURIComponent(branch)}/${i.path}`,
					}));
			} else if (isGitLab) {
				...
				if (!path) {
					throw new Error(
						"GitLab 仓库 URL 格式错误，例如 https://<your-gitlab-host>/<group>/<project>",
					);
				}
				...
				const tree = await gitlabApi<Array<{ path: string; type: string }>>(
					treeUrl,
					extractedToken,
				);
				...
				if (tree.length >= 100) {
					console.warn(
						`⚠️ GitLab: 文件数量达到API限制(100)，可能有文件未被扫描。建议使用排除模式减少文件数。`,
					);
				}
			} else {
			...
			console.log(
				`✅ 任务 ${taskId}: 状态已更新为 running，total_files=${files.length}`,
			);
			...
			const worker = async () => {
				while (true) {
					...
					if (taskControl.isCancelled(taskId)) {
						console.log(
							`🛑 [检查点1] 任务 ${taskId} 已被用户取消，停止分析（在文件 ${current}/${files.length} 前）`,
						);
						return;
					}
					...
					if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
						console.error(
							`❌ 任务 ${taskId}: 连续失败 ${consecutiveFailures} 次，停止分析`,
						);
						throw new Error(
							`连续失败 ${consecutiveFailures} 次，可能是 LLM API 服务异常`,
						);
					}
					...
					if (
						totalFiles > 10 &&
						failedCount / totalFiles > MAX_TOTAL_FAILURES_RATIO
					) {
						console.error(
							`❌ 任务 ${taskId}: 失败率过高 (${Math.round((failedCount / totalFiles) * 100)}%)，停止分析`,
						);
						throw new Error(
							`失败率过高 (${failedCount}/${totalFiles})，建议检查 LLM 配置或切换其他提供商`,
						);
					}
					...
					try {
						...
						if (taskControl.isCancelled(taskId)) {
							console.log(
								`🛑 [检查点2] 任务 ${taskId} 在LLM分析完成后检测到取消，跳过保存结果（文件: ${f.path}）`,
							);
							return;
						}
						...
						for (const issue of issues) {
							await api.createAuditIssue({
								...
								ai_explanation: issue.xai
									? JSON.stringify(issue.xai)
									: issue.ai_explanation || null,
								status: "open",
								resolved_by: null,
								resolved_at: null,
							} as any);
						...
						// 每分析一个文件都更新进度，确保实时性
						console.log(
							`📈 ${repoType}任务 ${taskId}: 进度 ${totalFiles}/${files.length} (${Math.round((totalFiles / files.length) * 100)}%)`,
						);
						await api.updateAuditTask(taskId, {
							status: "running",
							total_files: files.length,
						...
						consecutiveFailures++;
						console.error(
							`❌ 分析文件失败 (${f.path}): [连续失败${consecutiveFailures}次, 总失败${failedCount}/${totalFiles}]`,
							fileError,
						);
					}
					await new Promise((r) => setTimeout(r, LLM_GAP_MS));
			...
			const pool = Array.from(
				{ length: Math.min(LLM_CONCURRENCY, files.length) },
				() => worker(),
			);
			...
			const qualityScore =
				createdIssues === 0 ? 100 : Math.max(0, 100 - createdIssues * 2);
